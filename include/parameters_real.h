#ifndef PARAMETERS_real_H
#define PARAMETERS_real_H
...
class flagser_parameters_real {
public:
	flagser_parameters_real() { filtration_algorithm.reset(get_filtration_computer("zero")); }
	flagser_parameters_real(const named_arguments_t& named_arguments) {
    ...
    flagser_parameters real_params_to_standard(){
        // NB default constructor of flagser_parameters will reset it's filtration algorithm!
        flagser_parameters params = flagser_parameters();
        params.directed = true;
        params.nb_threads = 1;
        params.filtration_algorithm.reset(get_filtration_computer("max"));
        return params; 
    }

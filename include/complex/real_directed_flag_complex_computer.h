...
namespace real_directed_flag_complex_computer {
...
template <typename Complex> class coboundary_iterator_t {
	...
	short dimension;
	const real_compressed_sparse_matrix<real_entry_t>& matrix; //appears okay to just change the coefficients to double
	const index_t index;
	const real_coefficient_t coefficient;
	const coefficient_t modulus;
	size_t current_offset = 0;

public:
	coboundary_iterator_t(const Complex* _complex, short _dimension, const real_compressed_sparse_matrix<real_entry_t>& _matrix,
	                      index_t _index, real_coefficient_t _coefficient, coefficient_t _modulus)
	...
	filtration_entry_t next() {
		real_entry_t entry = *(matrix.cbegin(index) + current_offset++);
		real_coefficient_t coface_coefficient = get_coefficient(entry) * coefficient;// % modulus; //TODO: eliminate modulus
...
struct compute_filtration_t {
	...
	void operator()(vertex_index_t* first_vertex, int size) {
		...
		//std::cout << std::flush << "(called compute_filtration_t operator)" << std::flush;
		current_index++;
		directed_flag_complex_cell_t cell(first_vertex);
		...
		if (filtration_algorithm->needs_face_filtration()) {
			...
			for (int i = 0; i < size; i++) {
				...
				if (!cell_hash.size()) {
					...
					// std::string print_cell_bdy = "\ncell ";
					// print_cell_bdy += cell.to_string(size-1);
					// print_cell_bdy += " with boundary ";
					// print_cell_bdy += cell.boundary(i).to_string(size-2);
					// print_cell_bdy += "\n";
					// std::cout << print_cell_bdy << std::flush;
					//TODO: remove above debug print
...
class real_directed_flag_complex_computer_t {
...
public:
	...
	inline coboundary_iterator_t<real_directed_flag_complex_computer_t> coboundary(filtration_entry_t cell) {
		if (current_dimension > max_dimension || is_top_dimension()) {
			return coboundary_iterator_t<real_directed_flag_complex_computer_t>(this, current_dimension,
			                                                               coboundary_matrix[0], -1, 1, modulus);
		}

		size_t i = 0;
		while (i < nb_threads - 1 && index_t(coboundary_matrix_offsets[i + 1]) <= get_index(cell)) { i++; }
		return coboundary_iterator_t<real_directed_flag_complex_computer_t>(
...
struct real_store_coboundaries_in_cache_t {
	real_store_coboundaries_in_cache_t(real_compressed_sparse_matrix<real_entry_t>& _coboundary_matrix, int _current_dimension,
	    ...
	    : is_first(_is_first), current_dimension(_current_dimension), coboundary_matrix(_coboundary_matrix),
	      ...
	      total_cell_number(_total_cell_number), nb_threads(nb_threads) {}
	void done() {
#ifdef INDICATE_PROGRESS
		if (is_first)
			std::cout << "\033[K"
			...
			for (size_t offset = 0; offset < graph.incidence_row_length; offset++) {
				...
				while (bits > 0) {
					...
					coboundary_matrix.push_back(
						//the coefficient in the entry should be +/- 1
						// i & 1 is true if i is odd, false if i is even
						// so we just want to make entry with +/- 1
					    // real_make_entry(index_t(pair->second + cell_hash_offsets[thread_index]), i & 1 ? -1 + modulus : 1)); //TODO: eliminate modulus
						real_make_entry(index_t(pair->second + cell_hash_offsets[thread_index]), i & 1 ? -1 : 1));
				}
			}
		}
...
private:
	...
	real_compressed_sparse_matrix<real_entry_t>& coboundary_matrix;
	const filtered_directed_graph_t& graph;
	std::vector<cell_hash_map_t>& cell_hash;
...
void real_directed_flag_complex_computer_t::prepare_next_dimension(int dimension) {
	// if (dimension == 0) return; //Commented 2023-03-03 Ben Joness
...
// 	}

	{
		...
		std::vector<real_store_coboundaries_in_cache_t> store_coboundaries;
		for (size_t i = 0; i < nb_threads; i++) coboundary_matrix[i] = real_compressed_sparse_matrix<real_entry_t>();
		for (size_t i = 0; i < nb_threads; i++) {
			store_coboundaries.push_back(real_store_coboundaries_in_cache_t(
			    coboundary_matrix[i], dimension, graph, _cache_next_cells, _cache_next_cells_offsets,
			    int(cell_count[dimension]), i == 0, nb_threads));
		}
